...
lazy_static! {
  ...
  static ref SLUR_REGEX: Regex = {
    let slurs = r"(fag(g|got|tard)?\b|cock\s?sucker(s|ing)?|ni((g{2,}|q)+|[gq]{2,})[e3r]+(s|z)?|mudslime?s?|kikes?|\bspi(c|k)s?\b|\bchinks?|gooks?|bitch(es|ing|y)?|whor(es?|ing)|\btr(a|@)nn?(y|ies?)|\b(b|re|r)tard(ed)?s?)".to_string();
...
pub fn check_slurs(text: &str) -> Result<(), MyError> {
  if let Err(slurs) = slur_check(text) {
    Err(MyError::new(anyhow!(slurs_vec_to_str(slurs))))
  } else {
    Ok(())
  }
}

pub fn check_slurs_opt(text: &Option<String>) -> Result<(), MyError> {
  match text {
